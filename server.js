...
MongoClient.connect(url, {
    ...
    io.on('connection', socket => {
        if (!socket.isAuthorized) {
            io.to(socket.id).emit('feedback', JSON.stringify({
                error: false,
                data: 500
            }));
        }
        ...
        socket.on('send to bulletin', (userRequest) => {
            ...
            try {
                ...
                io.to(socket.id).emit('feedback', JSON.stringify({
                    error: true,
                    mssg: 'invalid send message request'
                }));
                socket.disconnect();
                return;
            }
                ...
                if (socket.isAuthorized) {
                    ...
                    if (socket.rooms.hasOwnProperty(userRequest.bulletin)) {
                        ...
                        let toSend = {
                            ...
                            data: 105,
                            payload: {
                                bulletin_id: userRequest.bulletin,
                                messages: queue.getMessages(userRequest.bulletin)
                            }
                        };
                        io.to(socket.id).emit('feedback', JSON.stringify(toSend));
                        queue.getMessagesDB(userRequest.bulletin, (data) => {
                            let toSendDB = {
                                error: false,
                                data: 105,
                                payload: {
                        ...
                        io.to(socket.id).emit('feedback', JSON.stringify({
                            error: true,
                            mssg: 'auth failed bulletin not subscribed'
                        }));
                        socket.disconnect();
                    }
                }
